...
try:
    ...
    #compile
    command = "-v %s npclown/gcc:2.0 sh -c 'cd /data; make'" % (volumn)
    ...
    #run
    runName = getFromDict(key='runName', json_data=json_data, default='')

    #command = "-v %s npclown/gcc:2.0 sh -c '%s < %s'" % (volumn, imageName, dirpath+runName, dirpath+stdin)
    command = "-v %s npclown/gcc:2.0 sh -c '%s < /data/stdin.txt; rm -f %s /data/stdin.txt'" % (volumn, "/data/"+runName,"/data/"+runName)
